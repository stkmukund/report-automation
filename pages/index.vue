...
export default {
  ...
  methods: {
    ...
    async rebillRev() {
      try {
        let response = await axios.get(
          `/api/order/rebill-revenue/?startDate=${this.startDate}&endDate=${this.endDate}`
        );
        this.campaignData.map((k, i) => {
    ...
    async declinedVip() {
      try {
        ...
        const fetch = async (value, index) => {
          const { data } = await axios.get(
            `/api/purchases/declined-vip/?startDate=${this.startDate}&endDate=${this.endDate}&campaignId=${value}`
          );
          const { totalResults } = data;
          this.campaignData[index].declinedVip = Number(totalResults);
          return 1;
        };
        for (const [index, value] of categoryCampaignIds.entries()) {
          await fetch(value, index);
    ...
    async totalVip() {
      try {
        let response = await axios.get(
          `/api/order/total-vip/?startDate=${this.startDate}&endDate=${this.endDate}`
        );
        this.campaignData.map((k, i) => {
