...
const fetchSales = async (campaignProductId,campaignId) => {
  ...
  let response = await fetch(`https://api.checkoutchamp.com/order/query/?loginId=revboostapirs.nymbus&password=RSsfFrR2nN5PcC6L1pSRs&campaignId=${queryStringCampaign}&orderStatus=COMPLETE&startDate=06/01/2024&endDate=07/01/2024&resultsPerPage=200&campaignProductId=${queryStringProduct}&orderType=NEW_SALE&paySource=CREDITCARD`);
