...
export default defineEventHandler(async (event) => {
  ...
  const totalActiveVip = async () => {
    const { data } = await axios.post(
      `https://api.checkoutchamp.com/purchase/query/?loginId=${config.CC_LOGIN_ID}&password=${config.CC_PASSWORRD}&campaignId=${campaignIdString}&status=ACTIVE&startDate=01/01/2010&endDate=${query.endDate}&resultsPerPage=1`
    );
